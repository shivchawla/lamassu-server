...
function toCashInTx (row) {
  ...
  return _.set('direction', 'cashIn', newObj)
}
...
function fetchDeviceTx (deviceId, phone, page, perPage) {

  const ifPhone = phone != "" ? "and phone=$2 limit $3 offset $4" : "limit $2 offset $3";

  const sql = `select * from cash_in_txs
    where device_id=$1 and (error_code is NULL or error_code!='operatorCancel') ${ifPhone}`

  const values = phone != "" ? [deviceId, phone, perPage, (page - 1)*perPage] : [deviceId, perPage, (page - 1)*perPage]

  return db.any(sql, values)
    .then(_.map(toCashInTx))
    .then(txs => {
