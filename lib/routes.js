...
function cancelTx(req, res, next) {
  if (!req.query.cancel) return res.status(400).send({Error: 'Requires cancel'})

  const reasonInQuery = _.get('reason', req.query);
  
  const reasonCode = reasonInQuery == 'payment'? 'paymentError' : '';  
  const reason = reasonInQuery == "payment" ? 'Payment Error' : '';  

  console.log("Reason: ", reason);
  console.log("Reason Code: ", reasonCode);

  return Tx.cancel(req.params.id, {reason, reasonCode})
  .then(r => res.json(r))
  .catch(err => {
    console.log(err);
    throw err;
...
//New added code to -- Shiv
function getDeviceTx (req, res, next) {
  if (req.deviceId) {
    const phone = _.get('phone', req.query);
    const page = _.get('page', req.query);
    const perPage = _.get('perPage', req.query);

    return helpers.fetchDeviceTx(req.deviceId, phone, page, perPage)
      .then(r => res.json(r))
      .catch(next)
  }
...
function processPaymentTx(req, res, next) {
  const {tx, amount, cardData} = req.body;
  
  const idenEmpresa = "M213727879";
  const token = "mz1BrvntKfxVcYREg41v";  
  const key_public = "65Of37bbkEPPlcACs0McnVJVVIhkrVEtx3DdMlnx";
  const key_secret = "qy34oBcI4MwvEmWw4XmpuZbPsyZUOpuxt5IPwKLc";  
  const postUrl = `https://app.pagalocard.com/api/v1/integracion/${token}`;

  const empresa = {idenEmpresa, key_public, key_secret};
  const firstName = cardData["nameCard"].split(" ")[0] || "skipped";
  const lastName = cardData["nameCard"].split(" ")[1] || "skipped";

  //Country//Email//IP are important and no * in name
  const cliente = {
      Total:parseInt(amount), 
      fecha_transaccion: moment.tz('America/Guatemala').format('YYYY-MM-DD HH:mm:ss'), 
      currency: tx.fiatCode,
      firstName:"skipped", lastName:"skipped", street1: "skipped",country:"GT",city:"skipped",state:"skipped", postalCode:"skipped", email:"skp@skp.com", ipAddress:"192.168.1.3", phone:tx.phone || "40428803", nit:"",deviceFingerprintID:""};
  
  const detalle = [{
    nombre: "Producto Digital",
    precio: parseInt(amount),
    cantidad: 1, 
    tipo:'Producto Digital',
    id_producto: Array.from(tx.cryptoCode.toLowerCase()).map(s => s.charCodeAt(0) - 97).join(''),
    Subtotal: parseInt(amount)
  }];

  const tarjetaPagalo = cardData;

  const paymentDetails = {
    empresa: JSON.stringify(empresa), 
    cliente: JSON.stringify(cliente), 
    detalle: JSON.stringify(detalle), 
    tarjetaPagalo: JSON.stringify(tarjetaPagalo)
  };
   
  const strData = JSON.stringify(paymentDetails);
  console.log("Payment Details - Stringified 1");
  console.log(strData);

  const headers = {"Content-Type":"application/json","Content-Length": strData.length}; 
  ...
  .then(r => {
    ...
    if (response.decision == "ACCEPT") {
      ...
      throw(new Error(`Payment failed; Reason: ${response.reasonCode}`))
    }
  })
  .catch(err => {
    console.log(err);
...
function notifyCashboxRemoval (req, res, next) {
  return Promise.all([machineLoader.getMachine(req.deviceId), settingsLoader.loadLatestConfig()])
    .then(([machine, config]) => {
      const cashInSettings = configManager.getCashInSettings(config)
      const reply = () => res.status(200).send({ status: 'OK' })
      return cashInSettings.cashboxReset !== AUTOMATIC
        ? reply()
        : cashbox.createCashboxBatch(req.deviceId, machine.cashbox)
...
app.put('/tx/:id', cancelTx)

app.get('/tx', getPhoneTx)

//Added to get transactions for device
app.get('/txs', getDeviceTx)


//Added to processPayment
app.post('/pay', processPaymentTx)
